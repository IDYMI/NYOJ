...
const getters = {
  isTrainingAdmin: (state, getters, _, rootGetters) => {
    return rootGetters.isAuthenticated && (state.training.author === rootGetters.userInfo.username || rootGetters.isSuperAdmin || rootGetters.isNormalAdmin || state.groupTrainingAuth == 5);
  },
    ...
    return new Promise((resolve, reject) => {
      api.getTraining(rootState.route.params.trainingID).then(
        (res) => {
          ...
          commit('changeTraining', {
            training: training,
          });
          if (training.gid) {
            dispatch('getGroupTrainingAuth', {
              gid: training.gid,
            });
          }
          if (training.auth == TRAINING_TYPE.Private.name) {
            dispatch('getTrainingAccess', {
              auth: TRAINING_TYPE.Private.name,
            });
          }
        },
        (err) => {
