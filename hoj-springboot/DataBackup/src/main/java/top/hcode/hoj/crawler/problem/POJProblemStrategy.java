...
public class POJProblemStrategy extends ProblemStrategy {
        ...
        public RemoteProblemInfo getProblemInfo(String problemId, String author) throws Exception {
                ...
                info.setDescription(ReUtil.get(
                                "<p class=\"pst\">Description</p><div class=.*?>([\\s\\S]*?)</div><p class=\"pst\">",
                                html, 1)
                                .replaceAll("src=\"[../]*", "src=\"" + HOST + "/"));

                info.setInput(ReUtil.get("<p class=\"pst\">Input</p><div class=.*?>([\\s\\S]*?)</div><p class=\"pst\">",
                                html, 1));
                ...
                info.setHint(ReUtil.get("<p class=.*?>Hint</p><div class=.*?>([\\s\\S]*?)</div><p class=\"pst\">", html,
                                1));
