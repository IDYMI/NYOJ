...
CREATE TABLE `contest` (
  ...
  `auth` int(11) NOT NULL COMMENT '0为公开赛，1为私有赛（访问有密码），2为保护赛（提交有密码），3为正式赛（访问要报名）， 4为同步公开赛， 5为同步私有赛（访问有密码）',
  ...
  `sign_duration` bigint(20) DEFAULT NULL COMMENT '报名时长(s)',
  `max_participants` int(11) DEFAULT NULL COMMENT '队员上限(最大为3)',
  `gmt_create` datetime DEFAULT CURRENT_TIMESTAMP,
  `gmt_modified` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`,`uid`),
  KEY `uid` (`uid`),
...
CREATE TABLE `discussion` (
  ...
  KEY `pid` (`pid`),
  CONSTRAINT `discussion_ibfk_1` FOREIGN KEY (`uid`) REFERENCES `user_info` (`uuid`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `discussion_ibfk_2` FOREIGN KEY (`category_id`) REFERENCES `category` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
